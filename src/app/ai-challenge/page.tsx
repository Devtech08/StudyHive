...
import type { ChallengeQuiz } from '@/ai/flows/generate-challenge-quiz';
import type { Question } from '@/ai/flows/generate-mock-exam'; // Re-use Question type
import { ArrowLeft, Check, Clock, Trophy } from 'lucide-react';

export default function AiChallengePage() {
  const [challenge, setChallenge] = useState<ChallengeQuiz | null>(null);
  const [loading, setLoading] = useState(true);
  const router = useRouter();

  useEffect(() => {
    const challengeData = localStorage.getItem('aiChallenge');
    if (challengeData) {
      try {
        const parsedChallenge = JSON.parse(challengeData);
        setChallenge(parsedChallenge);
      } catch (error) {
        console.error("Failed to parse AI challenge data from localStorage", error);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-muted/20">
      <header className="px-4 lg:px-6 h-16 flex items-center bg-background/80 backdrop-blur-sm sticky top-0 z-10 border-b">
        ...
        <div className="flex-1 text-center text-lg font-semibold flex items-center justify-center gap-2">
